...
local on_attach = function(_, bufnr)
  ...
  nmap('<C-h>', require('nvim-tmux-navigation').NvimTmuxNavigateLeft, 'Navigate Left')
  nmap('<C-j>', require('nvim-tmux-navigation').NvimTmuxNavigateDown, 'Navigate Down')
  nmap('<C-k>', require('nvim-tmux-navigation').NvimTmuxNavigateUp, 'Navigate Up')
  nmap('<C-l>', require('nvim-tmux-navigation').NvimTmuxNavigateRight, 'Navigate Right')
  nmap('<C-\\>', require('nvim-tmux-navigation').NvimTmuxNavigateLastActive, 'Navigate Last Active')
  nmap('<C-Space>', require('nvim-tmux-navigation').NvimTmuxNavigateNext, 'Navigate Next')
...
function Pymol_visual_xyz_convert()
  ...
  for line in coords:gmatch("[^\r\n]+") do
    ...
    local el, x, y, z = line:match("([%a]+)%s+([%d%.%-]+)%s+([%d%.%-]+)%s+([%d%.%-]+)")
...
function Pymol_visual_xyz_bohr_convert()
  ...
  local new_coords = ""
  for line in coords:gmatch("[^\r\n]+") do
    -- Match element symbols (letters) instead of atomic numbers
    local el, x, y, z = line:match("([%a]+)%s+([%d%.%-]+)%s+([%d%.%-]+)%s+([%d%.%-]+)")
    if el and x and y and z then
...
local normal_mappings = {
  {
    ...
    { "<leader>eL",   ":set nowrap<bar>vs<bar>e " .. nvim_config_path .. "/lsp-setup.lua<cr>", desc = "Edit lsp (split)" },
    { "<leader>el",   ":set nowrap<bar>e" .. nvim_config_path .. "/lua/lsp-setup.lua<cr>",     desc = "Edit lsp" },
    { "<leader>eO",   ":e" .. opencode_config_path .. "/opencode.json<cr>",                    desc = "Edit Opencode (split)" },
    { "<leader>ea",   ":e" .. opencode_config_path .. "/agent<cr>",                    desc = "Edit Opencode (split)" },
    ...
    { "<leader>et",   ":e" .. xdg_config_home .. "/tmux/tmux.conf.local<cr>",                  desc = "Edit Tmux" },
